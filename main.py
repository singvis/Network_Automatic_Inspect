...
if __name__ == '__main__':
    ...
    net.rich.printTable(title="按功能选择对应的序号", columns=table["colums"], rows=table["rows"])

    choice_function = input("请选择: ")
    if choice_function == '1':
        """测试连接"""
        net.rich.startLine(message="连接网络设备")
        net.execute_connect()
        net.rich.endLine(message="连接网络设备")
    elif choice_function == '2':
        """采集设备配置"""
        net.rich.startLine(message="采集设备配置信息")
        net.execute_getConfig()
        net.rich.endLine(message="采集设备配置信息")
